...
var lines = input.Split(Environment.NewLine).Select(s => s = s + ".").ToArray();
var found = new List<(int x, int y, int partNumber)>();
	...
	for (int x = 0; x < line.Length; x++)
	{
		...
		else
		{
			if (numStart != -1)
			{
				...
				var partNumber = int.Parse(line.Substring(numStart, numEnd - numStart + 1));
				bool hasSiblingMark = false;

				(int x, int y)? f = CheckMarkInRectangle(numStart - 1, y - 1, numEnd + 1, y + 1);

				if (f is not null)
				{
					found.Add((f.Value.x, f.Value.y, partNumber));
				}
			}
			numStart = -1;
			numEnd = -1;
...
var result = found
	.GroupBy(f => (f.x, f.y))
	.Where(g => g.Count() > 1)
	.Sum(g => g.First().partNumber * g.Skip(1).First().partNumber);

Console.WriteLine(result);

(int x, int y)? CheckMarkInRectangle(int x1, int y1, int x2, int y2)
{
	for (int y = y1; y <= y2; y++)
	{
		for (int x = x1; x <= x2; x++)

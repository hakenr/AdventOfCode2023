	...
	for (int x = 0; x < line.Length; x++)
	{
		...
		else
		{
			if (numStart != -1)
			{
				...
				hasSiblingMark = CheckMarkInRectangle(numStart - 1, y - 1, numEnd + 1, y + 1);
				Console.WriteLine($"{partNumber} {hasSiblingMark}");
...
bool CheckMarkInRectangle(int x1, int y1, int x2, int y2)
{
	for (int y = y1; y <= y2; y++)
	{
		for (int x = x1; x <= x2; x++)

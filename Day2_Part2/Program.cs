...
{
	var gameNumber = Convert.ToInt32(Regex.Replace(line, @"Game (\d+):.*", "$1"));
	var sets = Regex.Replace(line, @".*?: ", String.Empty).Split("; ");
	...
	foreach (string set in sets)
	{
		...
		{
			var segments = color.Trim().Split(" ");
			var count = Convert.ToInt32(segments[0]);
	...
	Console.WriteLine($"{gameNumber}: {reds.Max()} reds, {greens.Max()} greens, {blues.Max()} blues");
	sum += reds.Max() * greens.Max() * blues.Max();
}

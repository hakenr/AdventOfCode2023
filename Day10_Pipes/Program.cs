...
var map = new MapPoint[lines.Length, lines[0].Length];
...
Dictionary<char, List<(int X, int Y)>> directions = new()
{
	['.'] = [],
	['7'] = [(-1, 0), (0, 1)],
	['-'] = [(-1, 0), (1, 0)],
...
// find longest path back to start
var maxLength = 0;
...
while (stack.Count > 0)
{
	...
	var currentPoint = map[current.X, current.Y];
	if (currentPoint.IsVisited)
	{
		continue;
	}
	currentPoint.IsVisited = true;
	foreach (var direction in directions[currentPoint.Value])
	{
		...
		var nextPoint = map[next.X, next.Y];
		if (!directions[nextPoint.Value].Contains((direction.X * -1, direction.Y * -1)))
		{
			// musí být připojený na nás
			continue;
		}
		if (nextPoint.Value == 'S')
		{
			maxLength = Math.Max(maxLength, currentPoint.Distance + 1);
		}
...
void DumpMap(MapPoint[,] map)
{
	for (int y = 0; y < map.GetLength(0); y++)
	{
		for (int x = 0; x < map.GetLength(1); x++)
		{
			Console.Write($"[{map[x, y].Value}{map[x, y].Distance}]");
		}
		Console.WriteLine();
	}
}

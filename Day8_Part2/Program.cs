...
foreach (var line in lines.Skip(2))
{
	...
	var left = children[0].Substring(1, 3);
	var right = children[1].Substring(0, 3);
	nodes.Add(node, (left, right));
}

string[] starts = nodes.Keys.Where(k => k.EndsWith("A")).ToArray();
int positionInDirections = 0;
int[] stepsForNode = new int[starts.Length];
for (int i = 0; i < starts.Length; i++)
{
	...
	while (!current.EndsWith("Z"))
	{
		...
		var direction = directions[positionInDirections];
		if (direction == 'L')
		{
			current = left;
		}
		...
		positionInDirections++;
		if (positionInDirections == directions.Length)
		{
			positionInDirections = 0;
		}
		steps++;
	}
...
// least common multiple for stepsForNode
long lcm = 1;
foreach (var steps in stepsForNode)
{
	lcm = LCM(lcm, steps);
}
...
long LCM(long lcm, long steps)
{
	return (lcm * steps) / GCD(lcm, steps);
}
